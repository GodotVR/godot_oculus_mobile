...
float get_ipd(OvrMobileSession *session) {
    return check_session_initialized<float>(
            session,
            [&]() {
...
ovrVector3f get_controller_pose_properties(OvrMobileSession *session,
                                           int controller_id,
                                           get_pose_properties get_func) {
    return check_session_initialized<ovrVector3f>(
            session,
            [&]() {
                ovrVector3f scaled_property = {};
                const OvrMobileController *controller = session->get_ovr_mobile_controller();
                if (controller) {
                    const OvrMobileController::ControllerState *state =
...
ovrVector3f get_controller_angular_velocity(OvrMobileSession *session, int controller_id) {
    return get_controller_pose_properties(
            session, controller_id, [](const ovrRigidBodyPosef &head_pose) {
...
ovrVector3f get_head_angular_velocity(OvrMobileSession *session) {
    return get_head_pose_properties(
            session, [](const ovrRigidBodyPosef &head_pose) { return head_pose.AngularVelocity; });
}
...
ovrVector3f get_head_linear_velocity(OvrMobileSession *session) {
    return get_head_pose_properties(
            session, [](const ovrRigidBodyPosef &head_pose) { return head_pose.LinearVelocity; });
}
