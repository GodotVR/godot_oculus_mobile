...
void register_gdnative_display_refresh_rate(void *handle) {
	...
	godot::nativescript_api->godot_nativescript_register_method(handle, kClassName, "get_supported_display_refresh_rates", attributes, method);

	method.method = &set_display_refresh_rate;
	godot::nativescript_api->godot_nativescript_register_method(handle, kClassName, "set_display_refresh_rate", attributes, method);
}
...
GDCALLINGCONV godot_variant get_supported_display_refresh_rates(godot_object *instance, void *method_data, void *p_user_data, int num_args, godot_variant **args) {
	CHECK_USER_DATA(
			...
			godot::api->godot_array_new(&gd_return_array);

			for (int i = 0; i < supported_refresh_rates.size(); i++) {
				godot_variant refresh_rate;
				godot::api->godot_variant_new_real(&refresh_rate, supported_refresh_rates[i]);
				godot::api->godot_array_push_back(&gd_return_array, &refresh_rate);
				godot::api->godot_variant_destroy(&refresh_rate);
			}

			godot::api->godot_variant_new_array(&ret, &gd_return_array);
			godot::api->godot_array_destroy(&gd_return_array);
...
GDCALLINGCONV godot_variant set_display_refresh_rate(godot_object *instance, void *method_data, void *p_user_data, int num_args, godot_variant **args) {
	CHECK_USER_DATA(
			const double refresh_rate = godot::api->godot_variant_as_real(args[0]);
			godot::api->godot_variant_new_bool(&ret, ovrmobile::set_display_refresh_rate(ovr_mobile_session, refresh_rate));
	)
}