...
void register_gdnative_hand_tracking(void *p_handle) {
	...
	{ // register all the functions that we want to expose via the OvrHandTracking class in GDScript
		...
		godot::nativescript_api->godot_nativescript_register_method(p_handle, kClassName, "get_hand_pose", attributes, method);

		method.method = &get_hand_scale;
		godot::nativescript_api->godot_nativescript_register_method(p_handle, kClassName, "get_hand_scale", attributes, method);
		
		method.method = &is_pointer_pose_valid;
		godot::nativescript_api->godot_nativescript_register_method(p_handle, kClassName, "is_pointer_pose_valid", attributes, method);
		method.method = &get_pointer_pose;
		godot::nativescript_api->godot_nativescript_register_method(p_handle, kClassName, "get_pointer_pose", attributes, method);
	}
}
...
GDCALLINGCONV godot_variant get_hand_pose(godot_object *p_instance, void *p_method_data, void *p_user_data, int p_num_args, godot_variant **p_args) {
	CHECK_OVR(
        godot::api->godot_variant_new_real(&ret, 0.0); // in case sth. fails below we return 0.0 to always have a float return value
			...
			if (pController != nullptr) {
				...
				if (pState != nullptr && pState->connected) {
					...
					if (godot::api->godot_array_size(&rotations) != ovrHandBone_Max) {
						...
						godot::api->godot_variant_new_real(&ret, return_value);
					}
				}
			}
		} // num_args check
	...
	CHECK_OVR(
		...
		if (p_num_args != 1) {
			...
			if (pController != nullptr) {
				const ovrmobile::OvrMobileController::ControllerState* pState = pController->get_controller_state_by_id(controller_id);
				if (pState != nullptr && pState->connected) {
					godot::api->godot_variant_new_real(&ret, pState->hand_pose.HandScale);
				}
			}
		}
    )
...
GDCALLINGCONV godot_variant is_pointer_pose_valid(godot_object *p_instance, void *p_method_data, void *p_user_data, int p_num_args, godot_variant **p_args) {
	CHECK_OVR(
        godot::api->godot_variant_new_real(&ret, false); // in case sth. fails below we return false
		if (p_num_args != 1) {
			...
			if (pController != nullptr) {
				...
				if (pState != nullptr && pState->connected) {
					if (ovrmobile::check_bit(pState->input_hand.InputStateStatus, ovrInputStateHandStatus_PointerValid)) {
						...
						godot::api->godot_variant_new_bool(&ret, false);
					}
				}
			}
		}
		...
		if (p_num_args != 1) {
			...
			if (pController != nullptr) {
				...
				if (pState != nullptr && pState->connected) {
					...
					godot::api->godot_variant_new_transform(&ret, &gd_transform);
				}
			}
		}
    )
