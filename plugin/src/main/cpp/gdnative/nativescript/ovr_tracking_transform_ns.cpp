...
void register_gdnative_tracking_transform(void *p_handle) {
	...
	{ // register all the functions that we want to expose via the OvrTrackingTransform class in GDScript
		...
		godot::nativescript_api->godot_nativescript_register_method(p_handle, kClassName, "get_tracking_space", attributes, method);

		method.method = &set_tracking_space;
		godot::nativescript_api->godot_nativescript_register_method(p_handle, kClassName, "set_tracking_space", attributes, method);
	}
}
...
GDCALLINGCONV godot_variant get_tracking_space(godot_object *p_instance, void *p_method_data, void *p_user_data, int p_num_args, godot_variant **p_args) {
	CHECK_USER_DATA(
		godot::api->godot_variant_new_int(&ret, ovrmobile::get_tracking_space(ovr_mobile_session));
	)	
}
...
GDCALLINGCONV godot_variant set_tracking_space(godot_object *p_instance, void *p_method_data, void *p_user_data, int p_num_args, godot_variant **p_args) {
	CHECK_USER_DATA(
		int space = godot::api->godot_variant_as_int(p_args[0]); // note that ovrTrackingSpace is an enum in VrApi.h
		godot::api->godot_variant_new_bool(&ret, ovrmobile::set_tracking_space(ovr_mobile_session, space));
	)	
}
